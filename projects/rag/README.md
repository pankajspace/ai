...
image, writes `~/secrets/rag.env`, adds the Nginx
`/rag/` location file under `/etc/nginx/conf.d/app-locations/` (with POST rate
...
per-project Compose service (`~/apps/rag/docker-compose.yml`) automatically
on every push.
...
```bash
docker compose -f ~/apps/rag/docker-compose.yml ps
docker compose -f ~/apps/rag/docker-compose.yml logs --tail=50 rag
cat ~/apps/rag/docker-compose.yml
```
...
List previous tags locally, then connect to EC2 and update `~/apps/rag/docker-compose.yml`:
...
```bash
docker compose -f ~/apps/rag/docker-compose.yml pull
docker compose -f ~/apps/rag/docker-compose.yml up -d
```
