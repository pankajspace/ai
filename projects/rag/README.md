...
1. **Create the ECR repository** (local):
   ...
   ```bash
   aws ecr create-repository --repository-name techtoday/rag --region us-east-1
   ```

2. **Seed the initial image** (local, from `projects/rag/`). Later pushes are
   ...
   cd projects/rag
   docker build --platform linux/amd64 -t $ECR/techtoday/rag:latest .
   docker push $ECR/techtoday/rag:latest
   ```
...
After this one-time setup, every push under `projects/rag/**` redeploys
automatically.
