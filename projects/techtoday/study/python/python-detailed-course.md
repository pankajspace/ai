...
`uv` is a fast Rust-based tool from Astral that can create virtual environments, install packages, resolve dependencies, and manage a `pyproject.toml` workflow. It complements the built-in `venv`: `uv venv` creates the environment, while `uv pip` installs into it without requiring activation.

Install `uv` with the official installer or your system package manager, then use it like this:
...
The important distinction is that `uv` can manage the complete workflow: `uv lock` records exact resolved versions in `uv.lock`, and `uv sync` makes the environment match that lock file. This makes a fresh checkout reproducible while keeping the human-edited dependency declarations in `pyproject.toml`.
