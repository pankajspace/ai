...
11. When the project is ready to document: keep project-specific values in its `README.md`; advance the next-port allocation in `ADD_PROJECT.md`; update shared-secret setup notes only if the shared process changed; and add a project card under `projects/techtoday/ai-projects.html` (and update `projects/techtoday/index.html` if featured on the homepage) if the project should be public.
